...
import { ErrorText, Loading, errMessage } from '../lib';
import { cachedPage, fetchPage, invalidateAll, type LoadedPage } from '../cache';
import { hrefHome, hrefNode, hrefPage, hrefSpace, navigate } from '../router';
import { collectAttachmentRefs } from '../markdown';
import { cachedImageUrls, revokeImageUrls } from '../attachment-cache';
...
export function PageView({
  ...
  // Not `useAsync`, because this view can start with an answer. A body already
  // held is shown on the same frame the page opens and the request
  // below runs behind it; only a page with nothing held shows the spinner.
  ...
  useEffect(() => {
    let live = true;
    const held = cachedPage(pageId);
    setPage(held);
    setError(null);
    setLoading(held === null);

    // Revalidated even on a hit: the stamp that justified the hit came from a
    // tree response that may itself be minutes old. The request settles what is
    // current; the held body only decides what is on screen while it does.
    fetchPage(pageId)
      .then((fresh) => {
        if (!live) return;
        setPage(fresh);
  ...
  if (loading) return <Loading label={t.page.loadingPage} />;
  ...
  if (error !== null) return <PageUnavailable spaceId={spaceId} error={error} />;
  ...
  const save = async (nextTitle: string, body: string) => {
    ...
    try {
      ...
      // The saved page is not the only stale entry: a rename moves the titles in
      // other pages' breadcrumbs too. Dropping everything is the rule.
      invalidateAll();
      setMode('view');
      reload();
      onStructureChange(); // the title may have changed the tree label
    } catch (e) {
