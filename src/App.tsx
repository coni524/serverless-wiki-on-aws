...
import { hrefAdmin, hrefHome, hrefNode, navigate, useRoute } from './router';
import { AdminNavigation } from './components/AdminNavigation';
import { ASSISTANT_DRAWER_ID, AiAssistant, useAssistant } from './components/AiAssistant';
import {
  NavigationResizer,
...
import type { NewNodeTarget } from './components/PageTree';
 ...
 * share is held here: `treeVersion` tells the tree its levels are stale after a
 * structural edit, and `expandPath` (the current page's ancestors, reported by
 * the page view) keeps the selected page unfolded. Neither is on the tree's
 * `key` — only the space is, because only the space makes it a different tree.
 */
export function App() {
  const t = useT();
   ...
   * title the open page is also showing. Remounting both is blunt but correct,
   * and it is what stops such a change from staying invisible until the browser
   * is reloaded.
   */
  const refetchAll = useCallback(() => {
    // First, and synchronously. Bumping the versions rebuilds the page view,
    // and it reads the body cache before the tree's response comes back — so a
    // cache dropped afterwards would have already handed over the body this
    // refetch exists to replace.
    invalidateAll();
  ...
  const signOut = useCallback(async () => {
    ...
    //
    // The cached attachment bytes are the one thing a reload does not take with
    // it, so they are dropped explicitly and before it.
