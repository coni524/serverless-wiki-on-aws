...
import { invalidateAll, noteFreshness } from '../cache';
import { useT } from '../i18n';
import { hrefNode, hrefSpace, navigate } from '../router';
...
/** The key `loaded` uses for the space root, which has no parent page id. */
const ROOT = '';
 ...
 * page arrives as `expandPath`, and staleness arrives as `refreshToken`. Bundling
 * them meant every step between pages threw away the reader's expanded branches
 * along with the fetched levels.
 */
export function PageTree({
  ...
  expandPath: string[];
  /** Changes when what was fetched is stale. Every visible level is fetched again. */
  refreshToken: number;
  ...
  const t = useT();
  const [loaded, setLoaded] = useState<Record<string, Level>>({});
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [expanded, setExpanded] = useState<string[]>(expandPath);
  // Keys already being fetched. Without it, the effect below would re-request a
  // level on every render until its response lands.
  const inFlight = useRef(new Set<string>());
  // Bumped by each refresh. A response carrying an older generation answers a
  // request that refresh has already replaced, and writing it back would restore
  // the very levels the refresh was called to discard.
  ...
  const load = (parentPageId: string | null, cursor?: string) => {
    const key = parentPageId ?? ROOT;
    if (cursor === undefined && inFlight.current.has(key)) return;
    inFlight.current.add(key);
    const gen = generation.current;
    // The cursor argument is passed only when there is one: an explicit
    // `undefined` crosses the wire as `null`, which the API reads as a real
    // cursor and answers with nothing.
    const request =
      cursor === undefined
        ? api.listChildPages(spaceId, parentPageId)
        : api.listChildPages(spaceId, parentPageId, cursor);
    request
      .then((result) => {
        if (generation.current !== gen) return;
        inFlight.current.delete(key);
        // Every row carries the revision and the update time already. Recording
        // them here is what lets the page view decide, with no request of its
        // own, whether the body it holds is still the current one.
        noteFreshness(result.items);
        setLoaded((prev) => ({
          ...prev,
          [key]: {
            items: cursor === undefined ? result.items : [...(prev[key]?.items ?? []), ...result.items],
      ...
      .catch((e) => {
        if (generation.current !== gen) return;
        inFlight.current.delete(key);
        setErrors((prev) => ({ ...prev, [key]: errMessage(e) }));
      });
  };

  // Something the tree is showing has changed underneath it — a rename, a
  // delete, or a write by the assistant. Every fetched level is dropped, and the
  // levels that are on screen are fetched again by the effect at the foot of
  // this component; the expanded set is not touched, so the reader's tree is
  // still open where they left it.
  ...
  /**
   * Materialise the visible tree, and collect into `wanted` the key of every
   * level it had to draw as pending. Recursion follows expanded nodes only, and
   * stops at the depth cap or at an id already on the current path, so cyclic
   * data cannot loop (see MAX_TREE_DEPTH).
   *
   * Gathering the fetches here, rather than firing one from the toggle handler,
   * is what makes "a level is fetched when it appears on screen" the whole rule.
   * The first paint, a toggle, a page opened from a link, and a refresh all
   * reach the same list, so none of them needs a fetch of its own — and a level
  ...
  const build = (
    ...
    const nodes: TreeNode[] = sortSiblings(level.items).map((page) => {
      ...
      const below = loaded[page.pageId];
      const hasChildren =
      ...
      const children = open
        ? build(page.pageId, depth + 1, [...ancestors, page.pageId], wanted)
  ...
  const wanted: string[] = [];
  const roots = build(null, 0, [], wanted);

  // Fetch what the render above found missing. The dependency is the list
  // itself, so a render that wants nothing new asks for nothing; `refreshToken`
  // is there because a refresh has to re-ask even for a list that reads the
  // same as the one before it.
  //
  // The list is compared as JSON rather than as joined keys because `ROOT` is
  // the empty string: joined, "wants nothing" and "wants the space root" are
  // the same text, and the refresh that empties the tree would leave it saying
  // loading for good.
  const wantedKey = JSON.stringify(wanted);
  useEffect(() => {
    for (const key of wanted) load(key === ROOT ? null : key);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [wantedKey, refreshToken]);

  const tree =
    roots.length === 1 && roots[0].kind === 'pending' && roots[0].message === null ? (
      <StatusIndicator type="loading">{t.common.loading}</StatusIndicator>
    ) : loaded[ROOT]?.items.length === 0 && newNode === null ? (
      ...
      <TreeView
        ...
        renderItem={(node) => {
          ...
          if (node.kind === 'more') {
            return {
              content: (
                <Link variant="primary" onFollow={() => load(node.parentPageId, node.cursor)}>
                  {t.space.loadMore}
                </Link>
              ),
