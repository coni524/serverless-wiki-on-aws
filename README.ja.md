...
- **ページは運用者自身のアカウントに置いたままにできます。** この Wiki は運用者の AWS アカウントの中へデプロイします。ほかの誰かが控えを持つことも、別の利用企業と同じテーブルを共有することもありません。DynamoDB のテーブルと S3 のバケットは `pnpm run destroy` でも消しません。運用者のデータなので、消すかどうかは運用者が決めます
- **人も AI エージェントも同じ権限判定を通ります。** Wiki を AI につなぐと、たいていは全部を見せたうえで「余計なことはしないでほしい」と頼む形になります。この Wiki では、ブラウザーの画面、Bedrock の AI アシスタント、MCP のクライアント、Obsidian の同期プラグインが、どれもサインインしたアカウントのスペース権限を API 層でリクエストごとに解決します。AI もほかと同じ呼び出し元であり、判定する側は相手が AI かどうかを見ません
- **誰も使っていない間の費用はほとんどかかりません。** Lambda、オンデマンドの DynamoDB、S3 は、リクエストの数と保存したバイト数で課金します。誰も読んでいない Wiki にかかるのは保管料くらいです。5 人のチームが、夜通し動き続けるサーバーの代金を払うことにはなりません
...
初回のデプロイだけは 2 周かかりますが、その 2 周目は `pnpm run deploy` 自身が回します。
利用者が Wiki を開くアドレスは CloudFront の配信オリジンであり、1 周目のデプロイがそれを作るので、その 1 周目が Lambda に設定を渡す時点ではまだ存在しません。
そこで `pnpm run deploy` は、1 周目が作ったスタックの出力からアドレスを読み直し、それを Lambda へ渡すためにもう 1 周デプロイします。
2 周目で変わるのは Lambda の環境変数だけです。
アドレスが既にあるデプロイは 1 周で終わります。
...
- **最初の 1 人を入れるまでに CLI の操作が要ります。** デプロイ直後の運用者は、AWS CLI で Cognito にアカウントを作り、一度サインインしてから `pnpm run seed-admin` を実行します。自分の権限を上げる API を Wiki が持たないため、最後の手順はスクリプトです。
